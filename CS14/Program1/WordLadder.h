...
*/
  ...
  private:
/* Private helper function that passes in a const string& and returns bool. 
Checks to see if the string passed in is a valid word in the dictionary */
    bool checkDict(const string&); 
/*Takes a word out of the dictionary. Only used to take the starting word out */
    void takeOut(const string&);
/*Bool function that compares two words and sees if the words are only one letter off */
    bool oneOff(const string&, const string&);
};
